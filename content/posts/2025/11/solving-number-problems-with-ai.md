+++
date = '2025-11-16T23:00:00+00:00'
draft = true
...
As we suspect that we may have other problems we can apply a breadth first search approach to, we will make our breadth first search implementation generic over both the state representation `TState` and the description of each state traversal (how we get get some state `S` to some subsequent state `S'`) this is represented by `TStateTraversalDescription`.
